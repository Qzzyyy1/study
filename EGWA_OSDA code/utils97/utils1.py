...
from models.DCRN_02 import DCRN_02
...
def get_encoder(args, info):
    net = args.net

    if net == 'DCRN_02':
        model_g = DCRN_02(info['source'].bands_num, args.patch)
    elif net == 'vgg':
        model_g = VGGBase()
    elif 'resnet' in net:
        model_g = ResNetFc(in_channels=info['source'].bands_num, model_name=args.net)
    else:
...
def get_model_init(args, info, known_num_class, all_num_class):

    model_g = get_encoder(args, info)
    ...
    model_h = PredictionHead(in_dim=model_g.output_num(), out_dim=128)

    return model_g, model_e, model_c,model_h

def get_model(args, info, known_num_class, all_num_class, domain_dim=2, dc_out_dim=None):
    ...
    return model_g, model_c, model_e, model_dc,model_h
